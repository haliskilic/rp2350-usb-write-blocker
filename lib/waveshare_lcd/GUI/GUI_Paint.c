...
{
    /* DUZELTME (yerel yama): '>' karsilastirmasi X==Width / Y==Height
     * degerlerini geciriyordu (off-by-one) -> satir tasmasi. */
...
void Paint_Clear(UWORD Color)
{
    ...
    {
        /* DUZELTME (yerel yama): X dongusu bayt sayisi (WidthByte=2*W)
         * yerine piksel sayisi (WidthMemory) uzerinden donmeli. Orijinal
         * kod her karede tamponun 480 bayt otesine yazip bitisik statik
         * degiskenleri (ornegin logger durumunu) bozuyordu. */
        for (UWORD Y = 0; Y < Paint.HeightByte; Y++)
        {
