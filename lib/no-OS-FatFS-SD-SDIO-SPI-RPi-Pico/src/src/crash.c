...
static inline void reset() {
    /* DUZELTME (yerel yama): Orijinal kod debugger kontrolu yorumda
     * kaldigi icin kosulsuz __breakpoint() calistiriyordu. HardFault
     * isleyicisi icinde debugger yokken BKPT = ARM LOCKUP (cekirdek
     * kalici olarak durur, reboot olmaz). Dogrudan sistem reseti yap;
     * watchdog scratch registerlari korunur (teshis bilgisi kaybolmaz). */
#if !PICO_RISCV
        NVIC_SystemReset();
#else
