    ...
    while (blk_cnt) {
        ...
        if (prev_buffer_addr) {
            ...
            if (!chk_crc16(prev_buffer_addr, sd_block_size, prev_block_crc)) {
                DBG_PRINTF("%s: Invalid CRC received: 0x%" PRIx16 "\n", __func__,
                           prev_block_crc);
                /* DUZELTME (yerel yama): Orijinal kod burada TX/RX DMA
                 * ciftini CALISIR halde birakip donuyordu; sahipsiz RX
                 * DMA'si sonraki SPI FIFO okumalarinin baytlarini calarak
                 * zaman asimsiz SDK spi_read_blocking dongusunu sonsuza
                 * kadar kilitleyebiliyordu. Donmeden once aktarimi bitir
                 * (zaman asiminda wait_complete kanallari iptal eder). */
                (void)sd_spi_transfer_wait_complete(
                    sd_card_p,
                    calculate_transfer_time_ms(sd_card_p->spi_if_p->spi, sd_block_size));
                return SD_BLOCK_DEVICE_ERROR_CRC;
            }
        }
